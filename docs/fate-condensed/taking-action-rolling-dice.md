...
如果角色的行動面對的是固定的障礙、或者想改變的是世界而非角色或生物，他的行動就會面對靜態的難度等級。這些行動包括撬鎖、堵門和對敵方營地進行戰術評估。GM 可以決定特定形象（角色、情境或其他形象）是否存在來確證其難度。
...
通常你會援引你的角色形象。你也可以援引情境形象，或者對另一個角色的形象進行敵對援引。

### 使用絕技

絕技可以為你的擲骰帶來獎勵，只要能符合絕技中寫明的標準，例如情況、行動或使用的技能。你可能希望使用創造優勢在那些情況下引入相符的的形象。當你描述行動時，也要記得絕技的情況，為成功做好準備。

通常，絕技會在某個狹窄情況中給你一個 +2 的獎勵，不需任何費用；你能隨時使用它們。一些罕見的、特別強的絕技可能需要你花費命運點來使用它們。
...
GM 決定，要扔中修格斯實際上很容易，所以這是**工匠**擲骰——他能那麼快找到並將易燃物轉變成武器嗎？難度是良好（+3）。伊森有普通（+1）的工匠，但擲出<FateFont>0+++</FateFont>，得到傑出（+4）成果。
